 ...
 */
@WebServlet(name = "RemoveSavedPlayer", urlPatterns = {"/RemoveSavedPlayer"})
public class RemoveSavedPlayer extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String toRemoveLink = request.getParameter("link");
        try {
            HttpSession session = request.getSession(false);
            if (session != null && toRemoveLink != null) {
                String username = (String) session.getAttribute("USERNAME");
                List<String> fbrefLinks = (List<String>) session.getAttribute("FBREF");
                
                SavedPlayerDAO savedPlayerDAO = new SavedPlayerDAO();
                boolean delete = savedPlayerDAO.delete(username, toRemoveLink);
                if (delete) {
                    fbrefLinks.remove(toRemoveLink);
                    session.setAttribute("FBREF", fbrefLinks);
